...
static void *__alloc(struct bpf_mem_cache *c, int node)
{
	...
	if (c->percpu_size) {
		...
		void **obj = kmalloc_bpf(c->percpu_size, flags);
		// void *pptr = __alloc_percpu_gfp(c->unit_size, 8, flags);
		void *pptr = __alloc_percpu_gfp_bpf(c->unit_size, 8, flags);
	...
	return kmalloc_bpf(c->unit_size, flags);
}
...
int bpf_mem_alloc_init(struct bpf_mem_alloc *ma, int size, bool percpu)
{
	...
	if (size) {
		...
		pc = __alloc_percpu_gfp_bpf(sizeof(*pc), 8, GFP_KERNEL);
		if (!pc)
			return -ENOMEM;
...
static void destroy_mem_alloc(struct bpf_mem_alloc *ma, int rcu_in_progress)
{
	...
	copy = kmalloc_bpf(sizeof(*ma), GFP_KERNEL);
	if (!copy) {
