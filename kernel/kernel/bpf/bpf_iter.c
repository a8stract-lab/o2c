			    ...
			    loff_t *ppos)
{
	...
	if (!seq->buf) {
		...
		seq->buf = kvmalloc_node_bpf(seq->size, GFP_KERNEL, 0);
		if (!seq->buf) {
			err = -ENOMEM;
			goto done;
		}
...
int bpf_iter_reg_target(const struct bpf_iter_reg *reg_info)
{
	...
	tinfo = kmalloc_bpf(sizeof(*tinfo), GFP_KERNEL| __GFP_ZERO);
	if (!tinfo)
		return -ENOMEM;
			 ...
			 struct bpf_prog *prog)
{
	...
	link = kmalloc_bpf(sizeof(*link), GFP_USER | __GFP_NOWARN| __GFP_ZERO);
	if (!link)
		return -ENOMEM;
