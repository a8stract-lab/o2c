...
struct bpf_prog *bpf_prog_alloc_no_stats(unsigned int size, gfp_t gfp_extra_flags)
{
	...
	fp = __vmalloc_bpf(size, gfp_flags);
	if (fp == NULL)
		return NULL;

	// aux = kzalloc(sizeof(*aux), GFP_KERNEL_ACCOUNT | gfp_extra_flags);
	aux = kmalloc_bpf(sizeof(*aux), GFP_KERNEL_ACCOUNT | gfp_extra_flags | __GFP_ZERO);
	if (aux == NULL) {
		vfree(fp);
		return NULL;
	}
	// fp->active = alloc_percpu_gfp(int, GFP_KERNEL_ACCOUNT | gfp_extra_flags);
	fp->active = alloc_percpu_gfp_bpf(int, GFP_KERNEL_ACCOUNT | gfp_extra_flags);
	if (!fp->active) {
		vfree(fp);
		kfree(aux);
		return NULL;
			  ...
			  bpf_jit_fill_hole_t bpf_fill_ill_insns)
{
	...
	*rw_header = kvmalloc_node_bpf(size, GFP_KERNEL, 0);
	if (!*rw_header) {
					      ...
					      gfp_t gfp_extra_flags)
{
	...
	fp = __vmalloc_bpf(fp_other->pages * PAGE_SIZE, gfp_flags);
	if (fp != NULL) {
...
{
	if (prog_cnt)
		return kmalloc_bpf(sizeof(struct bpf_prog_array) +
				...
				__u32 __user *prog_ids, u32 cnt)
{
	...
	ids = kmalloc_bpf(cnt * sizeof(u32), GFP_USER | __GFP_NOWARN);
	if (!ids)
		return -ENOMEM;
