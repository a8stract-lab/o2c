...
static struct bpf_trampoline *bpf_trampoline_lookup(u64 key)
{
	...
	tr = kmalloc_bpf(sizeof(*tr), GFP_KERNEL| __GFP_ZERO);
	if (!tr)
		goto out;
#ifdef CONFIG_DYNAMIC_FTRACE_WITH_DIRECT_CALLS
	// tr->fops = kzalloc(sizeof(struct ftrace_ops), GFP_KERNEL);
	tr->fops = kmalloc_bpf(sizeof(struct ftrace_ops), GFP_KERNEL| __GFP_ZERO);
	if (!tr->fops) {
		kfree(tr);
		tr = NULL;
		goto out;
...
bpf_trampoline_get_progs(const struct bpf_trampoline *tr, int *total, bool *ip_arg)
{
	...
	*total = 0;
	...
	tlinks = kmalloc_bpf(BPF_TRAMP_MAX * sizeof(*tlinks), GFP_KERNEL);
	if (!tlinks)
...
static struct bpf_tramp_image *bpf_tramp_image_alloc(u64 key, u32 idx)
{
	...
	im = kmalloc_bpf(sizeof(*im), GFP_KERNEL| __GFP_ZERO);
	if (!im)
		goto out;
						     ...
						     int cgroup_atype)
{
	...
	shim_link = kmalloc_bpf(sizeof(*shim_link), GFP_USER| __GFP_ZERO);
	if (!shim_link)
		return NULL;
