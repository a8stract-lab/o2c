...
static struct map_iter *map_iter_alloc(struct bpf_map *map)
{
	...
	iter = kmalloc_bpf(sizeof(*iter), GFP_KERNEL | __GFP_NOWARN| __GFP_ZERO);
	if (!iter)
		goto error;

	// iter->key = kzalloc(map->key_size, GFP_KERNEL | __GFP_NOWARN);
	iter->key = kmalloc_bpf(map->key_size, GFP_KERNEL | __GFP_NOWARN| __GFP_ZERO);
	if (!iter->key)
		goto error;
...
static int bpf_init_fs_context(struct fs_context *fc)
{
	...
	opts = kmalloc_bpf(sizeof(struct bpf_mount_opts), GFP_KERNEL| __GFP_ZERO);
	if (!opts)
		return -ENOMEM;
