			     ...
			     u32 log_level, char __user *log_ubuf, u32 log_size)
{
	...
	btf = kmalloc_bpf(sizeof(*btf), GFP_KERNEL | __GFP_NOWARN | __GFP_ZERO);
	if (!btf) {
		err = -ENOMEM;
		goto errout;
	}
	...
	data = kvmalloc_node_bpf(btf_data_size, GFP_KERNEL | __GFP_NOWARN, 0);
	if (!data) {
		err = -ENOMEM;
		goto errout;
	}
...
struct btf *btf_parse_vmlinux(void)
{
	...
	// env = kzalloc(sizeof(*env), GFP_KERNEL | __GFP_NOWARN);
	env = kmalloc_bpf(sizeof(*env), GFP_KERNEL | __GFP_NOWARN | __GFP_ZERO);
	if (!env)
		return ERR_PTR(-ENOMEM);
...
static struct btf *btf_parse_module(const char *module_name, const void *data, unsigned int data_size)
{
	...
	btf->data = kvmalloc_node_bpf(data_size, GFP_KERNEL | __GFP_NOWARN, 0);
	if (!btf->data) {
		err = -ENOMEM;
		goto errout;
	}
...
ALLOW_ERROR_INJECTION(btf_release, NONE);
			     ...
			     void *module)
{
	...
	case MODULE_STATE_COMING:
		...
		btf_mod = kmalloc_bpf(sizeof(*btf_mod), GFP_KERNEL| __GFP_ZERO);
		if (!btf_mod) {
			err = -ENOMEM;
			goto out;
		}
		...
		if (IS_ENABLED(CONFIG_SYSFS)) {
			...
			attr = kmalloc_bpf(sizeof(*attr), GFP_KERNEL| __GFP_ZERO);
			if (!attr)
				goto out;
				  ...
				  struct btf_id_set8 *add_set)
{
	...
	if (!tab) {
		...
		tab = kmalloc_bpf(sizeof(*tab), GFP_KERNEL | __GFP_NOWARN| __GFP_ZERO);
		if (!tab)
			return -ENOMEM;
		   ...
		   int targ_start_id)
{
	...
	for (i = targ_start_id; i < n; i++) {
		...
		new_cands = kmalloc_bpf(sizeof_cands(cands->cnt + 1), GFP_KERNEL);
		if (!new_cands) {
			bpf_free_cands(cands);
			return ERR_PTR(-ENOMEM);
		}
		   ...
		   int relo_idx, void *insn)
{
	...
	specs = kmalloc_bpf(3 * sizeof(*specs), GFP_KERNEL);
	if (!specs)
		return -ENOMEM;
	...
	if (need_cands) {
		...
		if (cc->cnt) {
			...
			cands.cands = kmalloc_bpf(cc->cnt * sizeof(*cands.cands), GFP_KERNEL);
			if (!cands.cands) {
				err = -ENOMEM;
				goto out;
			}
