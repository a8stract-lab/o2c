			       ...
			       const struct bpf_verifier_state *src)
{
	...
	for (i = 0; i <= src->curframe; i++) {
		...
		if (!dst) {
			...
			dst = kmalloc_bpf(sizeof(*dst), GFP_KERNEL| __GFP_ZERO);
			if (!dst)
				return -ENOMEM;
					     ...
					     bool speculative)
{
	...
	elem = kmalloc_bpf(sizeof(struct bpf_verifier_stack_elem), GFP_KERNEL| __GFP_ZERO);
	if (!elem)
		goto err;
						...
						int subprog)
{
	...
	frame = kmalloc_bpf(sizeof(*frame), GFP_KERNEL| __GFP_ZERO);
	if (!frame)
		goto err;
...
static int add_kfunc_call(struct bpf_verifier_env *env, u32 func_id, s16 offset)
{
	...
	if (!tab) {
		...
		tab = kmalloc_bpf(sizeof(*tab), GFP_KERNEL| __GFP_ZERO);
		if (!tab)
			return -ENOMEM;
	...
	if (!btf_tab && offset) {
		...
		btf_tab = kmalloc_bpf(sizeof(*btf_tab), GFP_KERNEL| __GFP_ZERO);
		if (!btf_tab)
			return -ENOMEM;
			     ...
			     set_callee_state_fn set_callee_state_cb)
{
	...
	callee = kmalloc_bpf(sizeof(*callee), GFP_KERNEL| __GFP_ZERO);
	if (!callee)
		return -ENOMEM;
			  ...
			  bpfptr_t uattr)
{
	...
	info_aux = kmalloc_bpf(nfuncs * sizeof(*info_aux), GFP_KERNEL | __GFP_NOWARN);
	if (!info_aux)
		goto err_free;
...
next:
	...
	new_sl = kmalloc_bpf(sizeof(struct bpf_verifier_state_list), GFP_KERNEL| __GFP_ZERO);
	if (!new_sl)
		return -ENOMEM;
	env->total_states++;
	env->peak_states++;
					    ...
					    const struct bpf_insn *patch, u32 len)
{
	...
	if (len > 1) {
		...
		new_data = __vmalloc_bpf(array_size(env->prog->len + len - 1,
					      sizeof(struct bpf_insn_aux_data)), GFP_KERNEL | __GFP_ZERO);
		if (!new_data)
			return NULL;
	}
...
static int jit_subprogs(struct bpf_verifier_env *env)
{
	...
	func = kmalloc_bpf(env->subprog_cnt * sizeof(prog), GFP_KERNEL);
	if (!func)
		goto out_undo_insn;
...
static int do_check_common(struct bpf_verifier_env *env, int subprog)
{
	...
	state = kmalloc_bpf(sizeof(struct bpf_verifier_state), GFP_KERNEL| __GFP_ZERO);
	if (!state)
		return -ENOMEM;
	state->curframe = 0;
	...
	state->frame[0] = kmalloc_bpf(sizeof(struct bpf_func_state), GFP_KERNEL| __GFP_ZERO);
	if (!state->frame[0]) {
		kfree(state);
		return -ENOMEM;
	}
...
int bpf_check(struct bpf_prog **prog, union bpf_attr *attr, bpfptr_t uattr)
{
	...
	env = kmalloc_bpf(sizeof(struct bpf_verifier_env), GFP_KERNEL| __GFP_ZERO);
	if (!env)
		return -ENOMEM;
	log = &env->log;
	...
	env->insn_aux_data = __vmalloc_bpf(array_size(sizeof(struct bpf_insn_aux_data), len),  GFP_KERNEL | __GFP_ZERO);
	ret = -ENOMEM;
...
skip_full_check:
	...
	if (env->used_map_cnt) {
		...
		// 					  GFP_KERNEL);
		env->prog->aux->used_maps = kmalloc_bpf(env->used_map_cnt * sizeof(env->used_maps[0]),
							  GFP_KERNEL);
	...
	if (env->used_btf_cnt) {
		...
		// 					  GFP_KERNEL);
		env->prog->aux->used_btfs = kmalloc_bpf(env->used_btf_cnt * sizeof(env->used_btfs[0]),
							  GFP_KERNEL);
