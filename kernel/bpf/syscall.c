...
static void *__bpf_map_area_alloc(u64 size, int numa_node, bool mmapable)
{
	...
	if (mmapable) {
		...
		// 		    numa_node);
		area = kmalloc_bpf(size, gfp | GFP_USER | __GFP_NORETRY);
		if (area != NULL)
			return area;
	}

	return __vmalloc_node_range_bpf(size, align, VMALLOC_START, VMALLOC_END,
			   ...
			   int node)
{
	...
	ptr = kmalloc_bpf(size, flags | __GFP_ACCOUNT  | __GFP_ZERO);
				    ...
				    size_t align, gfp_t flags)
{
	...
	ptr = __alloc_percpu_gfp_bpf(size, align, flags | __GFP_ACCOUNT);
	set_active_memcg(old_memcg);
	mem_cgroup_put(memcg);
...
ALLOW_ERROR_INJECTION(bpf_map_release, NONE);
...
static int bpf_map_alloc_off_arr(struct bpf_map *map)
{
	...
	off_arr = kmalloc_bpf(sizeof(*map->off_arr), GFP_KERNEL | __GFP_NOWARN);
	if (!off_arr)
		return -ENOMEM;
	map->off_arr = off_arr;
...
static int map_lookup_elem(union bpf_attr *attr)
{
	...
	value = kvmalloc_node_bpf(value_size, GFP_USER | __GFP_NOWARN, 0);
	if (!value)
		goto free_key;
...
static int map_get_next_key(union bpf_attr *attr)
{
	...
	next_key = kvmalloc_node_bpf(map->key_size, GFP_USER, 0);
	if (!next_key)
		goto free_key;
			     ...
			     union bpf_attr __user *uattr)
{
	...
	key = kvmalloc_node_bpf(map->key_size, GFP_USER | __GFP_NOWARN, 0);
	if (!key)
		return -ENOMEM;
				    ...
				    union bpf_attr __user *uattr)
{
	...
	buf_prevkey = kvmalloc_node_bpf(map->key_size, GFP_USER | __GFP_NOWARN, 0);
	if (!buf_prevkey)
		return -ENOMEM;

	// buf = kvmalloc(map->key_size + value_size, GFP_USER | __GFP_NOWARN);
	buf = kvmalloc_node_bpf(map->key_size + value_size, GFP_USER | __GFP_NOWARN, 0);
	if (!buf) {
		kvfree(buf_prevkey);
		return -ENOMEM;
	}
...
ALLOW_ERROR_INJECTION(bpf_prog_release, NONE);
...
ALLOW_ERROR_INJECTION(bpf_link_release, NONE);
				   ...
				   u64 bpf_cookie)
{
	...
	link = kmalloc_bpf(sizeof(*link), GFP_USER| __GFP_ZERO);
	if (!link) {
		err = -ENOMEM;
		goto out_put_prog;
	}
...
static int bpf_prog_bind_map(union bpf_attr *attr)
{
	...
	// 			      GFP_KERNEL);
	used_maps_new = kmalloc_bpf((prog->aux->used_map_cnt + 1) * sizeof(used_maps_new[0]),
				      GFP_KERNEL);
	if (!used_maps_new) {
		ret = -ENOMEM;
		goto out_unlock;
