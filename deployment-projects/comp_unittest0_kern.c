...
{
    u64 addr =  - 0x7c90e820 + ctx->12 * 0x8;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r14 + ctx->10 * 0x1 - 0x8;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r14 + ctx->10 * 0x1 - 0x4;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r14 + ctx->10 * 0x1 - 0x2;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->ax + ctx->13 * 0x1 - 0x2;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r15 + ctx->13 * 0x1 - 0x8;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->ax + ctx->13 * 0x1 - 0x8;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r15 + ctx->13 * 0x1 - 0x4;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->ax + ctx->13 * 0x1 - 0x4;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r15 + ctx->13 * 0x1 - 0x2;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r13 + ctx->12 * 0x1 - 0x8;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->r9 + ctx->12 * 0x8 + 0x38;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->ax + ctx->11 * 0x1 + 0x18;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->ax + ctx->11 * 0x1 + 0x20;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->di + ctx->11 * 0x1 + 0x18;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->di + ctx->11 * 0x1 + 0x20;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->si + ctx->11 * 0x1 + 0x20;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->si + ctx->11 * 0x1 + 0x18;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->si + ctx->14 * 0x1 + 0x18;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->si + ctx->14 * 0x1 + 0x20;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->di + ctx->11 * 0x1 - 0x8;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->bx + ctx->15 * 0x1 - 0x4;
    sampling(addr, ctx->ip);
    return 0;
}
...
{
    u64 addr = ctx->bx + ctx->15 * 0x1 - 0x2;
    sampling(addr, ctx->ip);
    return 0;
}
